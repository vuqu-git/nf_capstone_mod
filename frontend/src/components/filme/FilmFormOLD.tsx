...
export default function FilmFormOLD({ filmItem, handleSubmit, onChange, formType }: Props) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (type === "checkbox") {
            parsedValue = (e.target as HTMLInputElement).checked; // Use `checked` for boolean values
        } else if (type === "number") {
            parsedValue = parseFloat(value); // Convert string to number for numeric inputs
        } else {
            parsedValue = value; // Default case for text and other inputs
        }

        // Update state with the parsed value
        onChange({ ...filmItem, [name]: parsedValue });
    };

    return (
        <>
            {/* heading based on formType */}
            {/*<h4 className="mb-4">{formType === "edit" ? "Edit an existing film" : "Add a new film"}</h4>*/}

            <Form.Label>{formType === "edit" ? "Edit an existing film" : "Add a new film"}</Form.Label>
            ...
            <Form onSubmit={(e) => handleSubmit(e, filmItem)}>
                ...
                <Button variant="primary" type="submit" className="mt-4">
                    {formType === "edit" ? "Update " : "Add "} film entry
                </Button>
            </Form>
        </>
    );
